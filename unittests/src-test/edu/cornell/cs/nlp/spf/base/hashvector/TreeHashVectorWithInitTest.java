...
public class TreeHashVectorWithInitTest {
	...
	public void test2() {
		...
		final TreeHashVector smallVector = new TreeHashVector();
		smallVector.set("1", "2", "3", "4", 3.0);
		smallVector.set("4", "3", "2", "1", 3.0);
		Assert.assertEquals(60.0, vector.dotProduct(smallVector), 0.0000001);
	}
	...
	public void test4() {
		final List<Thread> threads = new LinkedList<Thread>();
		for (int i = 0; i < 10; ++i) {
			threads.add(new Thread(new Test4Task()));
		}
		for (final Thread t : threads) {
			t.start();
		}
	...
	public void testSerialization() {
		...
		try {
			...
			final TreeHashVectorWithInit object = (TreeHashVectorWithInit) new ObjectInputStream(
	...
	private static class Test4Task implements Runnable {
